...
public class Tree<E extends Comparable<? super E>> {
    ...
    public void makeRoot(E x) {
        // todo
        root = makeRootRec (x, root);
    }

    private Node<E> makeRootRec(E x, Node<E> nodo) {
        if (nodo.value.equals(x))
        {
            return nodo;
        } else
        {
            if(x.compareTo(nodo.value) < 0)
            {
                if(nodo.left != null && x != nodo.value)
                {
                    nodo.left = makeRootRec(x, nodo.left);
                    nodo = rotateRight(nodo);
                }

            } else
            {
                if(nodo.right != null && x != nodo.value)
                {
                   nodo.right = makeRootRec(x, nodo.right);
