  ...
  ****************************************************************************/
  ...
  protected static <V> int size(Node<V> node) {
    // TODO
      ...
      for(Tuple2<Character,Node<V>> x : node.children.keysValues())
      {
           tam = tam + (sizeValue(x._2().value) + size (x._2()));
      }
  ...
  protected static <V> V search(String str, Node<V> node) {
    // TODO
      ...
      } else
      {
          char c = str.charAt(0);
          str = str.substring(1);
          return search(str, childOf(c, node));
  ...
  protected static <V> Node<V> insert(String str, V value, Node<V> node) {
    // TODO
      ...
      } else
      {
          ...
          if(childOf(c, node) == null)
          {
              node.children.insert(c, new Node());
          }

          insert(str, value, childOf(c, node));
  ...
  public List<String> strings() {
    ...
    return strings(root, str);
  }

  protected static <V> List<String> strings(Node<V> node, String str) {
    // TODO

      List<String> l = new ArrayList<>(); // Crea una lista nueva

      if(node.value != null)
      {
          l.append(str); //Agrega la cadena si el nodo no es null
      }

      for(Tuple2<Character, Node<V>> x: node.children.keysValues())
      {
          String childStr = str + x._1(); //Crea una nueva cadena
          List<String> palabrasPosibles = strings(x._2(), childStr);

          for (String palabras : palabrasPosibles) {
              l.append(palabras);
          }
  ...
  public static DictionaryStringTrie<Integer> fromList(List<String> list) {
    // TODO
      ...
      DictionaryStringTrie<Integer> d = new DictionaryStringTrie<>();
      int contador = 1;

      for(String x : list)
      {
          int cont = contador(x, list);
          d.insert(x, cont);
      }
  ...
  private static int contador (String pal,List<String> list)
  {
      int contador = 0;
