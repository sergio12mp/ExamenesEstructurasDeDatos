    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < graphs.length; i++) {
            ...
            if (ec.isEulerian()) {
                ...
                checkEulerianCycle(g, ec.eulerianCycle());
            } else {
        ...
        EulerianCycle<Character> ec = new EulerianCycle<>(g6);

    }

    private static <V> void checkEulerianCycle(Graph<V> g, List<V> cycle) {
        if (g.numEdges() != cycle.size() - 1) {
            System.err.println("wrong cycle length");
        }
        if (!(cycle.get(0)).equals(cycle.get(cycle.size() - 1))) {
            System.err.println("path is not closed");
        }
        for (int i = 0; i < cycle.size() - 2; i++) {
            V from = cycle.get(i);
            V to = cycle.get(i + 1);
            if (!g.successors(from).isElem(to)) {
                System.err.println("wrong edge: " + from + " -> " + to);
            }
        }

        V origin = cycle.get(0);
        if (g.degree(origin) != 2 * (occurrences(origin, cycle) - 1)) {
            System.err.println("wrong number of occurrences for origin vertex: " + origin);
        }
        for (V v : g.vertices()) {
            if (!v.equals(origin) && g.degree(v) != 2 * occurrences(v, cycle)) {
                System.err.println("wrong number of occurrences for vertex: " + v);
            }
        }
    }
