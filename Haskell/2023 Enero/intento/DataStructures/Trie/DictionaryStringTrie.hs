...
import GHC.Builtin.Types (charDataCon)
...
size Empty = 0;
--Version con foldL
--size (Node node dic) = foldl (\acc x -> acc + size x) (sizeValue node) (D.values dic)
size (Node node dic) = recu (D.values dic) (sizeValue node)
  where
    recu [] node = node
    recu (x:xs) node = recu xs (node+(size x))
...
childOf c (Node node dic)
  |c `elem` D.keys dic = fromJust (D.valueOf c dic)
  |otherwise = Empty
