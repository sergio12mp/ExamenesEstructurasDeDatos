...
get :: Int -> Vector a -> a
get i (V tam arbol)
  | i > (tam-1) || i < 0 = error "index out of bounds"
  | otherwise = getTree i arbol


getTree :: Int -> Tree a -> a 
getTree i (Leaf x) = x
getTree i (Node izq der)
  | mod i 2 == 0 = getTree (div i 2) izq
  | otherwise = getTree (div i 2) der
...
mapVector f (V tam arbol) = V tam (mapVectorArbol f arbol)

mapVectorArbol :: (a -> a) -> Tree a -> Tree a
mapVectorArbol f (Leaf x) = Leaf (f x)
mapVectorArbol f (Node izq der) = Node (mapVectorArbol f izq) (mapVectorArbol f der)
...
toList :: Vector a -> [a]
toList (V tam arbol) = toListArbol arbol

toListArbol :: Tree a -> [a]
toListArbol (Leaf x) = [x]
toListArbol (Node izq der) = intercalate (toListArbol izq) (toListArbol der)
...
fromList :: [a] -> Vector a
fromList xs
  | isPowerOfTwo (length xs) = V (length xs) (fromListArbol xs)
  | otherwise = error "No es potencia de 2" 


fromListArbol :: [a] -> Tree a 
fromListArbol [x] = Leaf x
fromListArbol (xs) = Node (fromListArbol primI) (fromListArbol secI)
  where
    (prim, sec) = splitAt (div (length xs) 2) xs
    interca = intercalate prim sec
    (primI, secI) = splitAt (div (length xs) 2) interca
