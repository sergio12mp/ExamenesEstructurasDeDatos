...
vector n x
  | n < 0 = error "El vector no puede ser negativo"
  | otherwise = V (2^n) (crearArbol n x)

crearArbol :: Int -> a -> Tree a
crearArbol n x = (Unif x)

-------------------------------------------------------------------------------
-- | Exercise b.

size :: Vector a -> Int
size (V tam arbol) = tam
...
simplify (Node izq1 der1) (Node izq2 der2) = Node (simplify izq1 der1) (simplify izq2 der2)
simplify (Unif x) (Node izq2 der2) = Node (Unif x) (simplify izq2 der2)
simplify (Node izq1 der1) (Unif x)= Node (simplify izq1 der1) (Unif x)
...
get i (V n (Unif x))
  | (i < 0) || ( i > (size (V n (Unif x))) ) = error "Vector fuera de rango"
  | otherwise = x
get i (V n (Node izq der))
  | i < 0 || i > (size (V n (Node izq der))) = error "Vector fuera de rango"
  | otherwise = getTree i ((size ((V n (Node izq der))))) (Node izq der)

getTree :: (Eq a) => Int -> Int -> Tree a -> a 
getTree i tam (Unif x) = x
getTree i tam (Node izq der)
  | i <= ((div tam 2)-1) = getTree i (div tam 2) izq
  | otherwise = getTree (i-(div tam 2)) (div tam 2) der
...
set i x (V n (Unif y))
  | i < 0 || i > (size (V n (Unif y))) = error "Vector fuera de rango"
  | otherwise = (V n (Unif x) )
set i x (V n (Node izq der))
  | i < 0 || i > (size (V n (Node izq der))) = error "Vector fuera de rango"
  | otherwise = ( V n (setTree i (size (V n (Node izq der))) x (Node izq der) ))

setTree :: (Eq a) => Int -> Int -> a -> Tree a -> Tree a
setTree i 1 x (Unif y) = (Unif x)
setTree i tam x (Unif y) = setTree i tam x (Node(Unif y)(Unif y))
setTree i tam x (Node izq der)
  | i <= ((div tam 2)-1) = simplify (setTree i (div tam 2) x izq) der
  | otherwise = simplify izq (setTree (i-(div tam 2)) (div tam 2) x der)
...
mapVector f (V tam arbol) = V tam (mapTree f arbol)

mapTree :: (Eq a, Eq b) => (a -> b) -> Tree a -> Tree b
mapTree f (Unif x) = (Unif (f x))
mapTree f (Node izq der) = simplify (mapTree f izq) (mapTree f der)

-------------------------------------------------------------------------------
-- | Exercise h.

filterVector :: (a -> Bool) -> Vector a -> [a]
filterVector pred (V tam arbol) = filterTree pred arbol []

filterTree :: (a -> Bool) -> Tree a -> [a] -> [a]
filterTree pred (Unif x) xs = if pred x then x:xs else xs 
filterTree pred (Node izq der) xs = (filterTree pred izq xs) ++ (filterTree pred der xs)

-------------------------------------------------------------------------------
-- | Exercise i.

depthOf :: (Eq a) => Int -> Vector a -> Int
depthOf i (V tam arbol) = depthOfTree i (size (V tam arbol)) arbol

depthOfTree :: (Eq a) => Int -> Int -> Tree a -> Int
depthOfTree i tam (Unif x) = 0
depthOfTree i tam (Node izq der)
  | i <= ((div tam 2)-1) = 1+depthOfTree i (div tam 2) izq
  | otherwise = 1+depthOfTree (i-(div tam 2)) (div tam 2) der
